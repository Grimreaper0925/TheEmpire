...
async function refreshDashboard(rootInteraction, cfg, guildId) {
    try {
        ...
        await InteractionHelper.safeEditReply(rootInteraction, {
            ...
            components: [...buildButtonRow(cfg, guildId), new ActionRowBuilder().addComponents(selectMenu)],
        });
    } catch (error) {
        ...
        try {
            ...
            collector.on('collect', async selectInteraction => {
                ...
                try {
                    ...
                    } else if (selectedOption === 'reset_interval_mins') {
...
async function handleResetChannel(selectInteraction, rootInteraction, cfg, settingsKey, client) {
    ...
    chanCollector.on('collect', async chanInteraction => {
        ...
        await sendEphemeralFollowUp(chanInteraction, { embeds: [successEmbed('Channel Updated', `Chat resets will now target ${channel}.`)] });
...
async function handleResetInterval(selectInteraction, rootInteraction, cfg, settingsKey, client) {
    ...
    try { await selectInteraction.showModal(modal); } catch { return; }
    ...
    if (isNaN(minutes) || minutes <= 0) {
        await replyUserError(submitted, { type: ErrorTypes.VALIDATION, message: 'Please enter a valid positive number.' });
        return;
    }
    ...
    await submitted.reply({ embeds: [successEmbed('Interval Updated', `Chat will now automatically reset every **${minutes}** minute(s).`)], flags: MessageFlags.Ephemeral });
    await refreshDashboard(rootInteraction, cfg, rootInteraction.guild.id);
}