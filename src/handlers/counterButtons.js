...
export const counterDeleteActionHandler = {
    ...
    async execute(interaction, client, args = []) {
        try {
            ...
            // Fallback direct key deletion since the separate file was missing
            const counterKey = `server_counter_${interaction.guild.id}_${counterId}`;
            await client.db.delete(counterKey).catch(() => {});

            await interaction.editReply({
                embeds: [successEmbed('Server counter successfully deleted.')],
                components: []
            }).catch(logger.error);
        } catch (error) {
